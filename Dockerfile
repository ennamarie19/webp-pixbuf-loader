...
##Build
RUN mkdir build
RUN CFLAGS=-fsanitize=fuzzer-no-link LDFLAGS=fsanitize=fuzzer-no-link CC=clang CXX=clang++ meson build -Dgdk_build_for_fuzz=enabled -Dgdk_pixbuf_query_loaders_path=/usr/lib/x86_64-linux-gnu/gdk-pixbuf-2.0/gdk-pixbuf-query-loaders
